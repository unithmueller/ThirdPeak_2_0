function [minv, maxv, gaussDat, kernelDat, newBinNmbrs] = plotInternMSDLyzer(Axes, binNumbers, SaveStructure, PixelTrackData, fitValue, property, dimension, isPixel, lengthUnit, pxsize, timeunit, timestep, filterIDs, performFit, autoBins)
...
%        end
       ...
       elseif property == "a"
           ...
           if isPixel
               ...
               txt = join([lengthUnit "²"],"");
               txt2 = join([timeunit "²"],"");
               xlabel(Axes, sprintf("MSD [%s/%s]",txt,txt2));
           end
       elseif property == "d"
