...
%----------------------------------------------------------------------------------------------------------------
%Remove gaussians that basically dont exist
hights = c;
filterIDX = (hights>0);
c = c(filterIDX);
newFilterIDX = zeros(size(parameter,1),1);
counter = 1;
for p = 1:size(filterIDX,1)
    if filterIDX(p) == 1
        newFilterIDX(counter:counter+1) = 1;
        counter = counter+2;
    else
        counter = counter+2;
    end
end
parameter = parameter(newFilterIDX==1);
% Now plot results.
...
tEstimate = table((1:size(c,1))', c(:), round(estimatedMuSigma(:, 1),2), round(estimatedMuSigma(:, 2),2), 'VariableNames', {'Number', 'Amplitude', 'Mean', 'Width'});
...
try
	...
	caption = sprintf('Estimation of %d Gaussian Curves that will fit data.', size(c,1));
	title(Axes, caption, 'Interpreter', 'none');
