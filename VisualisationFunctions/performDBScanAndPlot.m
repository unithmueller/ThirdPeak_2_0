function performDBScanAndPlot(app, data, flag3d, superpxsize, NmbrTimeWins, timeUnit, lengthUnit, superZsize, unit, propertyIDs, propertyNames, minCLusterSize, allPropId)
    %Perform a DBscan on the selected proeprties form the selection window,
    %then plot the clusters by thir xyz positions.
    a = 1
    ...
    %Do the hdbscan
    %make the hdbscan with the standart properties
    d = uiprogressdlg(app.VisualisationUIFigure,'Title','Performing HDBSCAN', 'Indeterminate','on');
    clusters = {};
    for k = 1:NmbrTimeWins
        if any(((propertyIDs > 53) & (propertyIDs < 58)))
            clusterData = tSegData{1,k}(:,propertyIDs);
            for tmpIDIdx = 1:size(propertyIDs,2)
                tmpID = propertyIDs(tmpIDIdx);
                if ((tmpID > 53) & (tmpID < 58))
                    diffTypeData = tSegData{1,k}(:,54:57);
                    [~,mIdx] = max(diffTypeData,[],2);
                    clusterData(:,tmpIDIdx) = mIdx;
                end
            end
        else
            clusterData = tSegData{1,k}(:,propertyIDs);
        end
        clusterGenerator = HDBSCAN(clusterData);
        clusterGenerator.run_hdbscan([],minCLusterSize);
        clusters{1,k} = clusterGenerator;
    end
    delete(d);
    ...
    if flag3d
        for j = 1:NmbrTimeWins
            tmpClusterDataLabel = clusters{1,j}.labels;
            tmpClusterDataP = clusters{1,j}.P;
            tmpMap = map;
            ...
            for i = 1:size(binX,1)
                if tmpClusterDataP(i) > 0.7
                    tmpMap{binX(i), binY(i), binZ(i)} = [tmpMap{binX(i), binY(i), binZ(i)}; tmpClusterDataLabel(i)];
                end
            end
            tmpMap = cellfun(@(x) determineMostinGroup(x), tmpMap, "UniformOutput",false);
        ...
        for j = 1:NmbrTimeWins
            ...
            for i = 1:size(binX,1)
                if tmpClusterDataP(i) > 0.7
                    tmpMap{binX(i), binY(i)} = [tmpMap{binX(i), binY(i)}; tmpClusterDataLabel(i)];
                end
            end
    ...
    %Prepare the labels
    propNames = {};
    for i = 1:size(propertyIDs,2)
        tmpidx = find(allPropId == propertyIDs(i));
        propNames{1,i} = propertyNames(tmpidx);
    end
    Titlename = join(string(propNames)," / ");
    ...
    for i = 1:NmbrTimeWins
        ...
        %hold off
        figure('Name', sprintf('Cluster Map - %s - TimeWindow %d', Titlename, i));
        if flag3d
            %3d
            clusterNmbrs = rmmissing(unique(data));
            uniClusterNmbrs = [1:1:size(clusterNmbrs,1)];
            for j = 1:size(clusterNmbrs,1)
                data(data(:,:) == clusterNmbrs(j)) = uniClusterNmbrs(j);
            end
            ...
            cmap = lines(size(clusterNmbrs,1));
            colormap(cmap);
            c = colorbar("Ticks",uniClusterNmbrs,"TickLabels",uniClusterNmbrs);
        ...
        %maybe also a scatter plot?
        hold off
        figure('Name', sprintf('Cluster Scatter Map - %s - TimeWindow %d', Titlename, i));
        
        if flag3d
            data = tSegData{1,i};
            clusterdata = clusters{1,i}.labels;
            uniqueClusters = unique(clusterdata);
            uniqueClusters = uniqueClusters(uniqueClusters > 0);
            cmap = lines(length(uniqueClusters));
            hold on
            for i = 1:length(uniqueClusters)
                idx = clusterdata==uniqueClusters(i);
                plot3(data(idx,3), data(idx,4), data(idx,5),".","color",cmap(i,:),"MarkerSize",5);
            end
            hold off
            xlabel(sprintf("X Position [%s]", lengthUnit));
            ylabel(sprintf("Y Position [%s]", lengthUnit));
            zlabel(sprintf("Z Position [%s]", lengthUnit));
            title(sprintf("%s" , Titlename));
        else
            data = tSegData{1,i};
            clusterdata = clusters{1,i}.labels;
            cmap = lines();
            s = gscatter(data(clusterdata > 0,3), data(clusterdata > 0,4), clusterdata(clusterdata > 0,1),cmap,".",[],"off");
            colormap(cmap);
            xlabel(sprintf("X Position [%s]", lengthUnit));
            ylabel(sprintf("Y Position [%s]", lengthUnit));
            title(sprintf("%s" , Titlename));
...
function a = determineMostinGroup(A)
    [B,BG] = groupcounts(A);
    [~,idx] = max(B);
    a = BG(idx);
end
