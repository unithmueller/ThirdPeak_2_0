    ...
    methods(Static)
        ...
        function diffmap = genRealDiffusionMap(grid, trajs, params)
            ...
            if flag3d == 0
                ...
                tmp = cellfun(@(x) [x(1), x(2:3)/x(1)],tempmap, "UniformOutput",false);
                tmp = cellfun(@(x) (x(2:3)-2*params.locPrecXY^2)/(params.frametime-(1/3*((params.expTime/params.frametime)*params.expTime))),tmp, "UniformOutput",false);
                tmp = cellfun(@(x) (x(1)+x(2)),tmp);
                diffmap = ScalarMap(grid, tmp, params, flag3d);
            ...
            elseif params.flag3d == 1
               ...
               for i = 1:size(tempmap,1)
                ...
                tmp = cellfun(@(x) [x(1), x(2:4)/x(1)],tempmap, "UniformOutput",false);
                tmp = cellfun(@(x) [(x(2:3)-2*params.locPrecXY^2)/(params.frametime-(1/3*((params.expTime/params.frametime)*params.expTime))), (x(4)-2*params.locPrecZ^2)/(params.frametime-(1/3*((params.expTime/params.frametime)*params.expTime)))],tmp, "UniformOutput",false);
                tmp = cellfun(@(x) (x(1)+x(2)+x(3)),tmp);
