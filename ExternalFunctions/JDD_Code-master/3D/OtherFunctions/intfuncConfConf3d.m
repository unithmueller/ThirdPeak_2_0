...
function[out]=intfuncConfConf3d(x1,x2,x3,N,yi,ri,dr,tau)
out=zeros(length(x1),length(x2),length(x3));
for i=1:length(x1)
    DConf1 = x1(i);
    z1=(4*pi*ri.^2*dr) - ((ri.^2)*dr/(2*sqrt(pi)*(DConf1*tau)^(3/2))).*exp(-ri.^2/(4*DConf1*tau));
    for j=1:length(x2)
        DConf2=x2(j);
        z2=(4*pi*ri.^2*dr) - ((ri.^2)*dr/(2*sqrt(pi)*(DConf2*tau)^(3/2))).*exp(-ri.^2/(4*DConf2*tau));
