...
function[out]=intfuncDConf3d(x1,x2,x3,N,yi,ri,dr,tau)
out=zeros(length(x1),length(x2),length(x3));
for i = 1:length(x1)
    D1 = x1(i);
    z1=dr*ri.^2/(2*sqrt(pi)*(D1*tau)^(3/2)).*exp(-ri.^2/(4*D1*tau));
    for j=1:length(x2)
        [i,j]
        D2=x2(j);
        z2=(4*pi*ri.^2*dr) - ((ri.^2)*dr/(2*sqrt(pi)*(D2*tau)^(3/2))).*exp(-ri.^2/(4*D2*tau));
