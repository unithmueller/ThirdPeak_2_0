...
function[out]=intfuncAnomConf2d(x1,x2,x3,x4,N,yi,ri,dr,tau)
...
for i=1:length(x2)
    ...
    for j=1:length(x3)
        ...
        z1=dr*ri/(Dalpha).*abs(integral(fun,min-1i*1e-6,-1*min-1i*1e-6,...
            'ArrayValued',true,'AbsTol',1e-6,'RelTol',1e-3));
        for k = 1:length(x1)
            DConf = x1(k);
            z2=(ri.*dr*2*pi)-((ri.*dr)/(DConf*tau)).*exp(-ri.^2/(4*DConf*tau));
