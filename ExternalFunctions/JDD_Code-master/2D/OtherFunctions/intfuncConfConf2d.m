...
function[out]=intfuncConfConf2d(x1,x2,x3,N,yi,ri,dr,tau)
out=zeros(length(x1),length(x2),length(x3));
for i=1:length(x1)
    DConf1 = x1(i);
    z1=(ri.*dr*2*pi) - ((ri.*dr)/(2*DConf1*tau)).*exp((-ri.^2)/(4*DConf1*tau));
    for j=1:length(x2)
        DConf2=x2(j);
        z2=(ri.*dr*2*pi) - ((ri.*dr)/(2*DConf2*tau)).*exp((-ri.^2)/(4*DConf2*tau));
