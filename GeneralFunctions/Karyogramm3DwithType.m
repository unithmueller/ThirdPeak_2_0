...
%next or beneath it. Can select the number of tracks to be plottet 
    ...
    %Check that there are enough tracks to work with
    ...
    for i = 1:amount
        ...
        if (typeselect == "All") && (dim(2) > 5) && mean(normalizedTracks(:,14)) ~= 0
    ...
    if flag3d
        title(sprintf("Karyogram in 3D of %s tracks", typeselect));
    else
        title(sprintf("Karyogram in 2D of %s tracks", typeselect));
    end
